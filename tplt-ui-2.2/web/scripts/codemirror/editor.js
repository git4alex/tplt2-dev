...
var Editor = (function () {
        ...
        function simplifyNode(node, top) {
            if (node.nodeType == 3) {
                ...
                if (!leaving && newlineElements.hasOwnProperty(node.nodeName.toUpperCase())) {
                    leaving = true;
                    if (!atEnd || !top) result.push(document.createElement("br"));
                }
            }
        }
    ...
    function traverseDOM(start) {
        ...
        function insertPart(part) {
            ...
            if (part.nodeType == 3) {
                ...
                if (afterBR && window.opera) point(makePartSpan(""));
                afterBR = true;
            }
            part.dirty = true;
            nodeQueue.push(part);
        ...
        function next() {
            ...
            if (partNode(node)) {
                ...
                if (afterBR && window.opera) node.parentNode.insertBefore(makePartSpan(""), node);
                nodeQueue.push(node);
                afterBR = true;
                return "\n";
            }
    ...
    function SearchCursor(editor, pattern, from, caseFold) {
        ...
        if (typeof pattern != "string") { // Regexp match
            this.matches = function (reverse, node, offset) {
                if (reverse) {
                    var line = getText(node).slice(0, offset),
                    ...
                    var line = getText(node).slice(offset),
                        match = line.match(pattern),
        ...
        function (reverse, node, offset) {
            var line = getText(node),
                len = pattern.length,
                match;
            if (reverse ? (offset >= len && (match = line.lastIndexOf(pattern, offset - len)) != - 1) : (match = line.indexOf(pattern, offset)) != - 1) return {
                from: {
                    node: node,
                    offset: match
                },
        ...
        function (reverse, node, offset) {
            ...
            if (reverse ? offsetA >= offset || offsetA != match.length : offsetA <= offset || offsetA != line.length - match.length) return;
            ...
            while (true) {
                ...
                return {
                    from: {
                        node: reverse ? pos : node,
                        offset: reverse ? offsetB : offsetA
                    },
                    to: {
                        node: reverse ? node : pos,
                        offset: reverse ? offsetA : offsetB
                    }
                };
            }
        };
    ...
    SearchCursor.prototype = {
        ...
        find: function (reverse) {
            ...
            var self = this,
                pos = reverse ? this.pos.from : this.pos.to,
                node = pos.node,
                offset = pos.offset;
        ...
        replace: function (string) {
            if (this.atOccurrence) {
                ...
                if (fragments) string = string.replace(/\\(\d)/, function (m, i) {
                    return fragments[i];
                });
    ...
    function Editor(options) {
        ...
        if (!options.readOnly && !internetExplorer) select.setCursorPos(container, {
            node: null,
            offset: 0
        });
        ...
        if (!options.readOnly) {
            ...
            function setEditable() {
                ...
                if (document.body.contentEditable != undefined && internetExplorer) document.body.contentEditable = "true";
                ...
                if (internetExplorer && options.height != "dynamic") document.body.style.minHeight = (
                window.frameElement.clientHeight - 2 * document.body.offsetTop - 5) + "px";

                document.documentElement.style.borderWidth = "0";
                if (!options.textWrapping) container.style.whiteSpace = "nowrap";
            }
            ...
            if (gecko) addEventHandler(window, "pagehide", function () {
                self.unloaded = true;
            });
            ...
            if (this.options.autoMatchParens) addEventHandler(document.body, "click", method(this, "scheduleParenHighlight"));
        }
    ...
    Editor.prototype = {
        ...
        importCode: function (code) {
            ...
            if (!this.options.incrementalLoading || lines.length < chunk) {
                ...
                function addChunk() {
                    ...
                    self.history.reset();
                    cur += chunk;
                    if (cur < lines.length) parent.setTimeout(addChunk, 1000);
                }
                addChunk();
            }
        },
        ...
        getCode: function () {
            ...
            if (webkit && this.container.lastChild.hackBR) accum.pop();
            webkitLastLineHack(this.container);
            return cleanText(accum.join(""));
        },

        checkLine: function (node) {
            if (node === false || !(node == null || node.parentNode == this.container || node.hackBR)) throw parent.CodeMirror.InvalidLineHandle;
        },
        ...
        cursorCoords: function (start, internal) {
            ...
            while (off) {
                ...
                if (off < txt.length) return withTempNode(txt.substr(0, off), function (tmp) {
            ...
            if (node && isSpan(node)) return measureFromNode(node, node.offsetWidth);
            else if (node && node.nextSibling && isSpan(node.nextSibling)) return measureFromNode(node.nextSibling, 0);
            else return withTempNode("\u200b", function (tmp) {
        ...
        setParser: function (name, parserConfig) {
            ...
            if (parserConfig && Editor.Parser.configure) Editor.Parser.configure(parserConfig);
        ...
        keyDown: function (event) {
            ...
            if (this.options.autoMatchParens) this.scheduleParenHighlight();
        ...
        keyPress: function (event) {
            ...
            // here.
            if ((this.frozen && (!this.keyFilter || this.keyFilter(event.keyCode || event.code, event))) || event.code == 13 || (event.code == 9 && this.options.tabMode != "default") || (event.code == 32 && event.shiftKey && this.options.tabMode == "default")) event.stop();
            else if (mac && (event.ctrlKey || event.metaKey) && event.character == "v") {
                this.reroutePasteEvent();
            }
            else if (electric && electric.indexOf(event.character) != - 1) parent.setTimeout(function () {
            ...
            // to the start of the line in Opera 10.60.
                ...
                if (event.code == 8) { // backspace
                    ...
                    if (sel !== false && next && isBR(next)) parent.setTimeout(function () {
                        if (select.selectionTopNode(self.container) == next) select.focusAfterNode(next.previousSibling, self.container);
                    }, 20);
                }
                    ...
                    if (sel && isBR(sel)) {
                        parent.setTimeout(function () {
                            if (select.selectionTopNode(self.container) != sel) select.focusAfterNode(sel, self.container);
                        }, 20);
                    }
                }
            }
            ...
            // the window.
            if (webkit && !this.options.textWrapping) setTimeout(function () {
                var node = select.selectionTopNode(self.container, true);
                if (node && node.nodeType == 3 && node.previousSibling && isBR(node.previousSibling) && node.nextSibling && isBR(node.nextSibling)) node.parentNode.replaceChild(document.createElement("BR"), node.previousSibling);
            }, 50);
        },
        ...
        indentLineAfter: function (start, direction) {
            ...
            var self = this,
                whiteSpace = whiteSpaceAfter(start);
            var newIndent = 0,
                curIndent = whiteSpace ? whiteSpace.currentText.length : 0;
            ...
            if (direction == "keep") {
                ...
                if (direction != null && this.options.tabMode != "indent") newIndent = direction ? curIndent + indentUnit : Math.max(0, curIndent - indentUnit)
                else if (start) newIndent = start.indentation(nextChars, curIndent, direction, firstText);
                else if (Editor.Parser.firstIndentation) newIndent = Editor.Parser.firstIndentation(nextChars, curIndent, direction, firstText);
            }
            ...
            // Not enough...
                ...
                // Otherwise, we have to add a new whitespace node.
                    ...
                    select.snapshotMove(firstText && (firstText.firstChild || firstText), whiteSpace.firstChild, newIndent, false, true);
                }
            }
        ...
        highlightAtCursor: function () {
            ...
            select.markSelection();
            if (this.highlight(pos, endOfLine(to, this.container), true, 20) === false) return false;
            select.selectMarked();
            return true;
        },
        ...
        handleTab: function (direction) {
            if (this.options.tabMode == "spaces" && !select.somethingSelected()) select.insertTabAtCursor();
        ...
        home: function () {
            var cur = select.selectionTopNode(this.container, true),
                start = cur;
            if (cur === false || !(!cur || cur.isPart || isBR(cur)) || !this.container.firstChild) return false;

            while (cur && !isBR(cur)) cur = cur.previousSibling;
            var next = cur ? cur.nextSibling : this.container.firstChild;
            if (next && next != start && next.isPart && hasClass(next, "whitespace")) select.focusAfterNode(next, this.container);
            else select.focusAfterNode(cur, this.container);
        ...
        pageUp: function () {
            var line = this.cursorPosition().line,
                scrollAmount = this.visibleLineCount();
        ...
        highlightParens: function (jump, fromKey) {
            ...
            function unhighlight(node) {
                if (!node) return;
                if (mark && !mark.call) removeClass(removeClass(node, mark[0]), mark[1]);
                else if (self.options.unmarkParen) self.options.unmarkParen(node);
                else {
            ...
            if (!(cursor && ((ch = paren(cursor)) || (cursor = cursor.nextSibling) && (ch = paren(cursor))))) return;
            ...
            while (true) {
                ...
                if (found.status == "dirty") {
                    ...
                    if (fromKey) parent.setTimeout(function () {
                        unhighlight(cursor);
                        unhighlight(found.node);
                    }, 500);
                    else self.highlighted = [cursor, found.node];
                    if (jump && found.node) select.focusAfterNode(found.node.previousSibling, this.container);
                    break;
                }
            }
        },
        ...
        indentRegion: function (start, end, direction, selectAfter) {
            var current = (start = startOfLine(start)),
                before = start && startOfLine(start.previousSibling);
            ...
            if (selectAfter) select.setCursorPos(this.container, {
                node: start,
                offset: 0
            }, {
                node: end,
        ...
        getDirtyNode: function () {
            while (this.dirty.length > 0) {
                ...
                try {
                    ...
                    if (found && (found.dirty || found.nodeType == 3)) return found;
                } catch (e) {}
            }
            return null;
        },
        ...
        highlightDirty: function (force) {
            ...
            while ((time() < endTime || force) && (start = this.getDirtyNode())) {
                ...
                if (result && result.node && result.dirty) this.addDirtyNode(result.node.nextSibling);
            }
        ...
        highlight: function (from, target, cleanLines, maxBacktrack) {
            ...
            if (!container.firstChild) return false;
            // Backtrack to the first node before from that has a partial
            // parse stored.
            while (from && (!from.parserFromHere || from.dirty)) {
                if (maxBacktrack != null && isBR(from) && (--maxBacktrack) < 0) return false;
                from = from.previousSibling;
            }
            // If we are at the end of the document, do nothing.
            if (from && !from.nextSibling) return false;
            ...
            function maybeTouch(node) {
                if (node) {
                    ...
                    if (lineDirty || old === undefined || node.nextSibling != old) self.history.touch(node);
                    node.oldNextSibling = node.nextSibling;
                }
                else {
                    var old = self.container.oldFirstChild;
                    if (lineDirty || old === undefined || self.container.firstChild != old) self.history.touch(null);
            ...
            function surroundedByBRs(node) {
                return (node.previousSibling == null || isBR(node.previousSibling)) && (node.nextSibling == null || isBR(node.nextSibling));
            }
            ...
            forEach(parsed, function (token) {
                ...
                if (token.value == "\n") {
                    ...
                    if (!isBR(part)) throw "Parser out of sync. Expected BR.";
                    ...
                    if ((endTime != null && time() >= endTime) || (!lineDirty && !prevLineDirty && lineNodes > 1 && !cleanLines)) throw StopIteration;
                    ...
                    if (!isSpan(part)) throw "Parser out of sync. Expected SPAN.";
