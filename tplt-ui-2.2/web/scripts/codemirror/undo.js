...
UndoHistory.prototype = {
    ...
    scheduleCommit: function () {
        ...
        this.commitTimeout = parent.setTimeout(function () {
            self.tryCommit();
        }, this.commitDelay);
    },
    ...
    chainNode: function (chains) {
        for (var i = 0; i < chains.length; i++) {
            var start = chains[i][0],
                node = start && (start.from || start.to);
            if (node) return node;
        }
    },
    ...
    notifyEnvironment: function () {
        ...
        if (window.frameElement && window.frameElement.CodeMirror.updateNumbers) window.frameElement.CodeMirror.updateNumbers();
    },
    ...
    shadowChain: function (chain) {
        var shadows = [],
            next = this.after(chain[0].from),
            end = chain[chain.length - 1].to;
        while (true) {
            shadows.push(next);
            var nextNode = next.to;
            if (!nextNode || nextNode == end) break;
            else next = nextNode.historyAfter || this.before(end);
    ...
    applyChain: function (chain) {
        ...
        for (var i = 0; i < chain.length; i++) {
            ...
            if (i > 0) self.container.insertBefore(line.from, end);
            ...
            if (cursor && cursor.node == line.from) {
                ...
                if (prev && i == chain.length - 1) {
                    ...
                    // the line.
                    for (var match = 0; match < cursor.offset && line.text.charAt(match) == prev.text.charAt(match); match++) {}
                    if (cursor.offset > match) cursordiff = line.text.length - prev.text.length;
                }
